...
ZKSYNC_PRICEFEED_ADDRESS := 0xfEefF7c3fB57d18C5C6Cdd71e45D2D0b4F9377bF
ACCOUNT = zksync
...
# For deploying to a specific network, we have to include the network flag `--network <NETWORK>`
# For example, to deploy to the sepolia network (make deploy-sepolia ARGS="--network sepolia")
ifeq ($(findstring --network sepolia,$(ARGS)),--network sepolia)
	NETWORK_ARGS := --rpc-url $(SEPOLIA_RPC_URL) --account $(ACCOUNT) --password .password --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif
ifeq ($(findstring --network holesky,$(ARGS)),--network holesky)
	NETWORK_ARGS := --rpc-url $(HOLESKY_RPC_URL) --account $(ACCOUNT) --password .password --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif
